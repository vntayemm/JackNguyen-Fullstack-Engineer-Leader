...
class NodeJSDomainValidatorService {
  constructor() {
    if (NodeJSDomainValidatorService.instance) {
      return NodeJSDomainValidatorService.instance;
    }
    NodeJSDomainValidatorService.instance = this;
  }
  ...
  async analyzeSPF(domain, spf_record) {
    try {
      ...
      const parsed = this.parseSPFRecord(record);
      return {
        domain,
        spf_record: record,
  ...
  async analyzeDMARC(domain, dmarc_record) {
    try {
      ...
      const parsed = this.parseDMARCRecord(record);
      return {
        domain,
        dmarc_record: record,
        parsed_record: parsed,
  ...
  // DNS records service
  async getDNSRecords(domain, record_type = 'TXT') {
    try {
...
const nodejsDomainValidatorService = new NodeJSDomainValidatorService();
export default nodejsDomainValidatorService;
