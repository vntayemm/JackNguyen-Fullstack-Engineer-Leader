  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">DNS Resolver</h1>
      ...
      {isLoading && (
        <div className="card">
          <div className="text-center">
            ...
            <p className="mt-2 text-gray-600 dark:text-gray-300">Resolving DNS records...</p>
          </div>
        </div>
      )}
      ...
      {error && (
        <div className="card error">
          ...
          <p className="text-gray-600 dark:text-gray-300">Failed to resolve DNS records. Please try again.</p>
        </div>
      )}

      {data && (
        <div className="card">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-4">DNS Resolution Results</h2>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-gray-900 dark:text-gray-400">{data.record_type}</span>
            </div>

            {data.records.length > 0 ? (
              <div>
                <span className="font-medium block mb-2 text-gray-700 dark:text-gray-300">Records:</span>
                  ...
                  {data.records.map((record, index) => (
                    <div key={index} className="bg-gray-50 dark:bg-gray-700 p-3 rounded border border-gray-200 dark:border-gray-600">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-sm font-medium text-gray-700 dark:text-gray-200">Record {index + 1}:</span>
                        {record.ttl && (
                          <span className="text-xs text-gray-500 dark:text-gray-400">TTL: {record.ttl}s</span>
                        )}
                      </div>
                      <code className="text-sm break-all text-gray-900 dark:text-gray-100">{record.value}</code>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="text-center py-4">
                <p className="text-gray-600 dark:text-gray-300">No {data.record_type} records found for this domain.</p>
              </div>
            )}
